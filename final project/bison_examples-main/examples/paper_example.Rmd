...
```{r}
fit <- sampling(model, model_data, cores=4, iter=2000, refresh=500)
```
...
log_p_samples <- exp(extract(fit)$log_p)
...
df_comparison <- data.frame(true=t(data$p)[lower.tri(data$p)], est=p_quantiles[2, ], est_lower=p_quantiles[1, ], est_upper=p_quantiles[3, ])

line_fit <- lm(est ~ true, df_comparison)
ggplot(df_comparison, aes(x=true, y=est)) +
  geom_point(color="#387780") +
  geom_errorbar(aes(ymin=est_lower, ymax=est_upper)) +
  geom_abline() +
  geom_point(x=0, y=0) +
  geom_abline(intercept=line_fit$coefficients[1], slope=line_fit$coefficients[2], color="#387780", linetype="dashed") +
